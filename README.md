...
4. **Crea il token:** Settings (profilo) → Developer settings → Personal access tokens → Fine-grained. Repository access: `piano-app` e `brain`. Permessi: Contents → Read and write su `piano-app`, Read su `brain`. Scadenza: 1 anno.
5. **Apri l'app**, Impostazioni: incolla il token, scrivi `utente/piano-app` e `utente/brain` (il campo «pipeline» si lascia vuoto: usa lo stesso repo `brain`). Salva. Il token resta solo nel browser del dispositivo.
